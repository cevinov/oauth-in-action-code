...
app.get("/authorize", function (req, res) {
  ...
  if (!client) {
    ...
    // The variables rscope and cscope represent the requested scope and the client's allowed scope, respectively. rscope is derived from the scope parameter in the
    // authorization request, split into an array of individual scopes. cscope is derived from the client's predefined scope, also split into an array.
    var rscope = req.query.scope ? req.query.scope.split(" ") : undefined;
    var cscope = client.scope ? client.scope.split(" ") : undefined;

    // checks if there are any scopes requested by the client that are not permitted for that client
    // This ensures that clients can only request scopes they are authorized for, maintaining proper scope restrictions and security
  ...
  if (req.body.grant_type == "authorization_code") {
    ...
    if (code) {
      ...
      if (code.request.client_id == clientId) {
        ...
        // The nosql.insert() method is used to store data into the NoSQL database. In this context, it saves the newly generated access token and refresh token along with the associated client ID and scope
        nosql.insert({
